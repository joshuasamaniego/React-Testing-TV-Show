...
//     render(<App />);
//     //negative assertion, so use query
// })

// test("Whether user can click dropdown to select a season", async () => {
    //     render(<App />);
...
test('Renders episodes when the API returns', async () => {
    ...
    // wait for the data to come back then test for the header (h1) component
    userEvent.click(await screen.findByText(/select a season/i))
    userEvent.click(screen.getByText(/season 1/i)) // state updater
